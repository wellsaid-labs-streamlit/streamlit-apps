""" A workbook to generating slurring testing survey.

Usage:
    $ PYTHONPATH=. streamlit run apps/model_evaluations/v11_1_slurring.py --runner.magicEnabled=false
"""
import io
import os
import time
...
gcs_csv_path_v11 = "wellsaid_labs_streamlit_data/model_evaluations/v11_1_slurring/v11/metadata_v11.csv"
gcs_csv_path_v11_1 = "wellsaid_labs_streamlit_data/model_evaluations/v11_1_slurring/v11_1/metadata_v11_1.csv"

gcs_responses_path = "wellsaid_labs_streamlit_data/model_evaluations/v11_1_slurring/results"
...
def load_subset():
    try:
        audio_csv_v11 = conn.read(gcs_csv_path_v11, input_format="csv")
        audio_csv_v11_1 = conn.read(gcs_csv_path_v11_1, input_format="csv")
    ...
    audio_csv = pd.concat([audio_csv_v11, audio_csv_v11_1])
...
def load_data():
    ...
    for form_audio_id in range(num_audio):
        ...
        subset_audio_id = audio_subset[form_audio_id]
        org_audio_id = audio_csv.iloc[subset_audio_id]["Id"]

        speaker = audio_csv.iloc[subset_audio_id]["Speaker"]
        formatted_speaker = format_speaker_name(speaker)
        style = audio_csv.iloc[subset_audio_id]["Session"].split(",")[2]
        script = audio_csv.iloc[subset_audio_id]["Script"]
        spectrogram_model = audio_csv.iloc[subset_audio_id]["Spectrogram Model"]
        signal_model = audio_csv.iloc[subset_audio_id]["Signal Model"]


        if spectrogram_model == "v11.pt":
            audio_path = os.path.join(gcs_audio_path_v11_1, f"{org_audio_id}.wav")
            audio_gcs = conn.open(audio_path, mode="rb")
            audio = io.BytesIO(audio_gcs.read())

        else:
            audio_path = os.path.join(gcs_audio_path_v11, f"{org_audio_id}.wav")
            audio_gcs = conn.open(audio_path, mode="rb")
            audio = io.BytesIO(audio_gcs.read())
